function[X_train,X_test]=nac_obr(m_train,m_test,rozmer) 
...
name=['M:\DP\zadanie1\skonvertovane_obrazky\train_full\macka.',int2str(k-1),'.jpg']; 
...
for k=1:m_train/2
%  nacitania obrazkov ako matic
name=['M:\DP\zadanie1\skonvertovane_obrazky\train_full\pes.',int2str(k-1),'.jpg']; 
A=imread(name); % nacitanie obrazku
[r1,r2,r3]=size(A); % rozmery matice
B=double(A)/255; % normalizacia dat rozmer 64x64x3
% transformacia 3D matice do  2D 
 ...
 for i=1:r1
     for j=1:r2
         train_set(r1*(i-1)+j,:,k+m_train/2)=B(i,j,:);
     end
...
for k=1:m_test/2
%nacitania obrazkov ako matic
name=['M:\DP\zadanie1\skonvertovane_obrazky\train_full\macka.',int2str(k-1+10000),'.jpg']; 
A=imread(name); % nacitanie obrazku
[r1,r2,r3]=size(A); % rozmery matice
B=double(A)/255;% normalizacia dat rozmer 64x64x3
...
for k=1:m_test/2
%  nacitania obrazkov ako matic
name=['M:\DP\zadanie1\skonvertovane_obrazky\train_full\pes.',int2str(k-1+10000),'.jpg']; 
A=imread(name); % nacitanie obrazku
[r1,r2,r3]=size(A); % rozmery matice
B=double(A)/255; % normalizacia dat rozmer 64x64x3
